    ...
    return (
        <div className="max-w-5xl mx-auto space-y-8">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-heading font-bold text-white mb-2">Take Attendance</h1>
                    <p className="text-gray-400">Class 10 - Section A | October 25, 2026</p>
                </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="glass p-5 rounded-2xl border border-white/5 flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white font-bold">{students.length}</div>
                    <span className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Total</span>
                </div>
                ...
                <div className="p-4 border-b border-white/5 flex items-center gap-4 bg-black/40">
                    <div className="relative flex-1 max-w-sm">
                        ...
                        <input
                            ...
                            className="w-full bg-card border border-white/10 rounded-lg py-2 pl-10 pr-4 text-sm text-white focus:outline-none focus:border-brand-purple transition-colors"
                        />
                    </div>
                        ...
                        <tbody className="divide-y divide-white/5">
                            {students.map((student, i) => (
                                <motion.tr
                                    ...
                                    key={student.id} className="hover:bg-white/5 transition-colors group"
                                >
                                    <td className="p-4 pl-6 font-bold text-gray-500">{student.roll}</td>
                                    ...
                                    <td className="p-4 text-gray-400 text-sm">STU-{student.id}</td>
                                    <td className="p-4 text-right pr-6">
                                        <div className="inline-flex bg-black/40 rounded-lg p-1 border border-white/5 overflow-hidden">
                                            <button
                                                onClick={() => setStatus(student.id, "Present")}
                                                className={`flex items-center justify-center w-10 h-8 rounded-md transition-all ${student.status === 'Present' ? 'bg-green-500 text-white shadow-[0_0_10px_rgba(34,197,94,0.3)]' : 'text-gray-500 hover:text-white hover:bg-white/5'}`}
                                                title="Present"
                                            ><Check className="w-4 h-4" /></button>

                                            <button
                                                onClick={() => setStatus(student.id, "Absent")}
                                                className={`flex items-center justify-center w-10 h-8 rounded-md transition-all ${student.status === 'Absent' ? 'bg-red-500 text-white shadow-[0_0_10px_rgba(239,68,68,0.3)]' : 'text-gray-500 hover:text-white hover:bg-white/5'}`}
                                                title="Absent"
                                            ><X className="w-4 h-4" /></button>

                                            <button
                                                onClick={() => setStatus(student.id, "Late")}
                                                className={`flex items-center justify-center w-10 h-8 rounded-md transition-all ${student.status === 'Late' ? 'bg-yellow-500 text-white shadow-[0_0_10px_rgba(234,179,8,0.3)]' : 'text-gray-500 hover:text-white hover:bg-white/5'}`}
                                                title="Late"
