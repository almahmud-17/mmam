    ...
    return (
        <div className="min-h-screen pt-32 pb-20 px-4">
            <div className="max-w-7xl mx-auto space-y-12">
                ...
                {/* Toolbar */}
                <div className="flex flex-col md:flex-row items-center justify-between gap-6 bg-white/5 p-4 rounded-3xl border border-white/10 backdrop-blur-md">
                    <div className="relative w-full md:max-w-md">
                        ...
                        <input
                            ...
                            className="w-full bg-black/40 border border-white/10 rounded-2xl py-3.5 pl-12 pr-6 text-white text-sm focus:outline-none focus:border-brand-purple transition-all placeholder:text-gray-600"
                        />
                    </div>
                    <div className="flex items-center gap-4 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
                        {["All", "Events", "Campus", "Sports", "Academic"].map((cat) => (
                            <button
                                key={cat}
                                className="whitespace-nowrap px-6 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/5 text-gray-400 hover:text-white text-sm font-semibold transition-all"
                            >
                                {cat}
                            </button>
                        ))}
                ...
                {isLoading ? (
                    <div className="flex flex-col items-center justify-center py-40 gap-4">
                        ...
                        <p className="text-gray-400 font-medium">Coming into focus...</p>
                    </div>
                        ...
                        {filteredItems.map((item, i) => (
                            ...
                            >
                                ...
                                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col justify-end p-8">
                                    ...
                                    {item.description && (
                                        <p className="text-gray-300 text-sm line-clamp-2 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500 delay-75">
                                            {item.description}
                                        </p>
                                    )}
            ...
            {selectedImage && (
                ...
                >
                    ...
                    <button className="absolute top-8 right-8 text-white hover:text-brand-pink transition-colors">
                        <X className="w-8 h-8" />
                    </button>
                </motion.div>
            )}
